"""Ratter analyser util functions."""
...
MODULE_BLACKLIST_PATTERNS = {
    "(ratter|ratter\\..*)",
    "(package.ratter|packages.ratter\\..*)",
}
...
def get_basename_fullname_pair(
    ...
    if isinstance(node, ast.UnaryOp):
        _error_class = error.RatterUnaryOpInNameable
    elif isinstance(node, ast.BinOp):
        _error_class = error.RatterBinOpInNameable
    elif isinstance(node, Constant.__args__):
        _error_class = error.RatterConstantInNameable
    elif isinstance(node, Literal.__args__):
        _error_class = error.RatterLiteralInNameable
    elif isinstance(node, Comprehension.__args__):
        _error_class = error.RatterComprehensionInNameable
    else:
...
def is_name(name: Any) -> bool:
    ...
    See: `parse_ratter_results_from_annotation(...)`
    """
...
def parse_ratter_results_from_annotation(fn_def: AnyFunctionDef, context) -> FunctionIR:
    ...
    pos_args, named_args = parse_annotation("ratter_results", fn_def)
    ...
    if not is_set_of_names(named_args.get("sets")):
        __type_error("'ratter_results' expects set of names for 'sets'")

    if not is_set_of_names(named_args.get("gets")):
        __type_error("'ratter_results' expects set of names for 'gets'")

    if not is_set_of_names(named_args.get("dels")):
        __type_error("'ratter_results' expects set of names for 'dels'")

    if not all(is_name(c[0]) for c in named_args.get("calls")):
        __type_error("'ratter_results' LHS of 'calls' to be names")

    if not all(is_args(c[1]) for c in named_args.get("calls")):
        __type_error("'ratter_results' expects args for 'calls'")

    if not all(len(c) == 2 for c in named_args.get("calls")):
        __type_error("'ratter_results' expects two elements per call")
